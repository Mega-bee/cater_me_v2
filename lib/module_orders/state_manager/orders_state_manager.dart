...
import 'package:cater_me_v2/module_orders/ui/state/order_detailes_success.dart';
  ...
  getOrders(OrdersScreenState screenState) {
    ...
    _ordersRepository.getOrders().then((value) {
      if (value == null) {
        ...
        emit(OrdersListSuccess(screenState: screenState, ordersList: occList));
      } else {
        emit(ErrorState(
  ...
  getOrdersDetails(OrdersDetailsScreenState screenState, String id) {
    ...
    _ordersRepository.getOrder(id).then((value) {
      if (value == null) {
        ...
        emit(OrdersDetailsSuccess(model: response, screenState: screenState));
      } else {
        emit(ErrorState(
            errMsg: value.errorMessage,
            retry: () {
